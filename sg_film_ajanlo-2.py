...
def _split_pipe_db(value: Any) -> List[str]:
...
BRAIN_SYNONYMS: Dict[str, List[str]] = {
    "konnyu":           ["könny", "konny", "laza", "könnyű", "egyszerű", "egyszeru"],
...
def batch_generate_why(
    ...
    try:
        ...
        prompt = (
            ...
            + "- Legyen KONKRÉT: utalj a film hangulatára, sztorira, miért illik a nézőnek\n"
...
def rank_movies(
    mood: str, time_limit: int, brain: str, extra: str,
    offset: int, take: int, era: str = "all"
) -> List[Movie]:
    era_filters = {
    ...
    if len(filtered) < take:
        filtered = [m for score, m in scored]
...
def default_profile() -> Dict[str, Any]:
    return {
        "time": None, "mood": None, "brain": None,
        "extra": "", "ready": False, "history": [],
        "era": "all", "era_asked": False, "emotion_asked": False,
    }
...
def next_question(p: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    if "mood" in missing:
        return (
            ...
            [
                "Pörgős (akció, feszültség)",
                "Nyugis (laza, chill)",
                "Sötét (thriller, horror)",
                "Felemelő (inspiráló, dráma)",
                "Vicces (komédia, humor)",
                "Romantikus (szerelem, érzelem)",
            ],
        )
    ...
    if "brain" in missing:
        return (
            ...
            [
                "Könnyű (kikapcsolódás)",
                "Közepes (kis csavar)",
                "Elgondolkodtató (mély, összetett)",
            ],
        )
    ...
    if not p.get("emotion_asked"):
        ...
        intensity_questions = {
            "sotet": (
                ...
                [
                    "Enyhén sötét, nem ijeszt",
                    "Közepes — feszült de nem horror",
                    "Igazán durva és nyomasztó",
                    "Mindegy",
                ],
            ),
            "porgos": (
                ...
                [
                    "Tiszta akció, minimális vér",
                    "Közepes — van némi brutalitás",
                    "Brutális és nyers",
                    "Mindegy",
                ],
            ),
            "vicces": (
                ...
                [
                    "Ártatlan, mindenki nevet",
                    "Fekete humor, csípős poénok",
                    "Abszurd és őrült",
                    "Mindegy",
                ],
            ),
            "felemelo": (
                ...
                [
                    "Kicsit megható, de nem sírok",
                    "Erősen érzelmes, könnyeket csal",
                    "Inkább inspiráló mint érzelmes",
                    "Mindegy",
                ],
            ),
            "nyugis": (
                "Milyen hangulatú legyen a film?",
                [
                    "Vidám és szórakoztató",
                    "Szívmelengető és meghitt",
                    "Kalandos de könnyed",
                    "Mindegy",
                ],
            ),
            "romantic": (
                ...
                [
                    "Édes és boldog szerelem",
                    "Drámai és tragikus románc",
                    "Bonyolult kapcsolat sok fordulattal",
                    "Mindegy",
                ],
            ),
        }
        return intensity_questions.get(
            mood,
            (
...
_CHAT_SYSTEM_PROMPT = (
    ...
    "Cél: barátságos, rövid, természetes beszélgetés. "
    "NE ajánlj rögtön filmeket. Kérdezd ki sorban: hangulat, idő, elgondolkodtatóság. "
    "Stílus: sötét elegancia, laza, motiváló, max 1 emoji. "
...
def require_password():
    if not Config.SITE_PASSWORD:
        return
    if request.path in ("/login", "/logout", "/auth/google", "/auth/google/callback"):
        return
...
def api_trailer():
    ...
    yt_search = "https://www.youtube.com/results?search_query=" + _q(query)
...
def api_justwatch():
    ...
    return jsonify({"url": jw_url, "title": title, "year": year})
...
# ---------------------------------------------------------------------------
# API: /api/local_poster
# ---------------------------------------------------------------------------
@app.get("/api/local_poster/<filename>")
def api_local_poster(filename):
    import re as _re
    from flask import send_from_directory
    if not _re.match(r'^[a-zA-Z0-9_\-]+\.(jpg|png|jpeg)$', filename):
        return "Invalid", 400
    poster_dir = os.path.join(BASE_DIR, "posters")
    filepath   = os.path.join(poster_dir, filename)
    if os.path.exists(filepath):
        return send_from_directory(poster_dir, filename, max_age=86400)
    return api_poster()
...
def api_chat():
    try:
        ...
        # Extra kulcsszavak a gombválaszokhoz
        ...
        ACTIONS: Dict[str, Any] = {
            # Hangulat (zárójelekkel és anélkül is)
        ...
        elif low in EXTRA_KW:
            add_extra(EXTRA_KW[low])
            p["emotion_asked"] = True
            handled_by_button = True
 
        # Szöveg értelmezés ha nem gomb
        ...
        # Következő kérdés lekérése — next_question BEÁLLÍTJA p["ready"]=True ha kész
        q_text, chips = next_question(p)
 
        # OpenAI chat reply ha elérhető és még kérdezünk
        if not p.get("ready"):
            ai_reply = openai_chat_reply(p, user_msg)
            if ai_reply:
                q_text = ai_reply
        ...
        return jsonify({
            "assistant":    "Hiba történt, próbáld újra.",
            "quick_replies":["Reset"],
...
#poster-strip {
  ...
  gap:10px; padding:12px; max-height:640px; overflow-y:auto;
}
...
.poster-btns { display:flex; gap:4px }
.poster-btns button {
...
.poster-btns button:hover { border-color:var(--gold); background:rgba(200,168,75,.08) }
    ...
    <div class="card-head" style="flex-wrap:wrap;gap:8px">
      ...
      <div style="display:flex;gap:6px;align-items:center">
        <select id="era-filter" style="padding:5px 10px;border-radius:8px;border:1px solid var(--border2);background:rgba(14,18,24,.9);color:var(--text);font-size:12px;outline:none;cursor:pointer">
          <option value="all" selected>Minden kor</option>
          <option value="recent">Friss (2015+)</option>
          <option value="new">Modern (2000-2015)</option>
          <option value="classic">Klasszikus (1980-2000)</option>
          <option value="old">Régi (1980 előtt)</option>
        </select>
...
document.querySelectorAll('.mood-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    await loadMore();
    statusLine.textContent = 'mood=' + mood + ' · time≈' + state.time + 'min';
  });
});
...
function posterHTML(m) {
  ...
  let ytUrl   = 'https://www.youtube.com/results?search_query=' + encodeURIComponent(m.title + ' ' + m.year + ' official trailer');
  let ytEmbed = '';
  ...
  return '<div class="poster-card">'
    ...
    + '<button class="trailer-play-btn" data-url="' + esc(ytUrl) + '" data-embed="' + esc(ytEmbed) + '" data-title="' + esc(m.title) + '" data-year="' + esc(String(m.year || '')) + '" data-tmdb="' + esc(m.tmdb_id || '') + '">▶ Trailer</button>'
...
function bindButtons(scope) {
  ...
  scope.querySelectorAll('.trailer-play-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      btn.disabled    = true;
      try {
        const res  = await fetch('/api/trailer?title=' + encodeURIComponent(title) + '&year=' + encodeURIComponent(year) + '&tmdb_id=' + encodeURIComponent(tmdb));
        const data = await res.json();
        const url  = data.url || '';
        if(url.includes('watch?v=')) {
          window.open(url, '_blank', 'noopener');
        } else {
          window.open('https://www.youtube.com/results?search_query=' + encodeURIComponent(title + ' ' + year + ' official trailer'), '_blank', 'noopener');
        }
      } catch(e) {
  ...
  scope.querySelectorAll('.jw-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      btn.disabled    = true;
      try {
        const res  = await fetch('/api/justwatch?title=' + encodeURIComponent(title) + '&year=' + encodeURIComponent(year));
        const data = await res.json();
        if(data.url) window.open(data.url, '_blank', 'noopener');
      } catch(e) {
        window.open('https://www.justwatch.com/hu/kereses?q=' + encodeURIComponent(title), '_blank', 'noopener');
      } finally {
        btn.textContent = orig;
        btn.disabled    = false;
      }
...
async function loadMore() {
  ...
  try {
    const res  = await fetch(url);
    const data = await res.json();
    if(pillLoaded) pillLoaded.textContent = (data.total || 0) + ' film';
    const items = data.items || [];
    if(!items.length) {
      const el = document.createElement('div');
      el.className   = 'empty-state';
      el.textContent = 'Nincs több találat.';
...
async function send() {
  ...
  try {
    ...
    if(prof.ready === true && !state.ready) {
      state.ready  = true;
      state.offset = 0;
      posterStrip.innerHTML = '';
      await loadMore();
    } else if(prof.ready === true && state.ready) {
      state.offset = 0;
      posterStrip.innerHTML = '';
      await loadMore();
    }
...
/* ── User area (Google bejelentkezés) ── */
...
fetch('/api/debug').then(r => r.json()).then(d => {
  if(pillLoaded) pillLoaded.textContent = (d.movies_loaded || 0) + ' film';
}).catch(() => {});
})();
</script>
</body>
...
if __name__ == "__main__":
    ...
    log.info("Starting SG Film Ajánló on port %d", port)
    app.run(
        host  = "0.0.0.0",
        port  = port,
        debug = False,
